{ config, pkgs, lib, ...}:
{
  ...
  config.systemd.services.zerovpnClient = lib.mkIf config.zerovpn.client.enable {
    ...
    script = "${pkgs.zerovpn}/bin/0vpn-leaf ${config.zerovpn.interface} /etc/zerovpn-key ${config.zerovpn.client.serverName} ${config.zerovpn.client.serverHost} ${builtins.toString config.zerovpn.endpointPort} ${builtins.toString config.zerovpn.announcePort} ${builtins.toString config.zerovpn.announceInterval} ${config.zerovpn.name}";
  };
