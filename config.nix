  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf config.zerovpn.client.enable {
      ...
      systemd.services.zerovpnClient = {
        ...
        script = "${pkgs.zerovpn}/bin/0vpn-leaf ${config.zerovpn.interface} /etc/zerovpn-key ${config.zerovpn.client.serverName} ${config.zerovpn.serverHost} ${builtins.toString config.zerovpn.endpointPort} ${builtins.toString config.zerovpn.announcePort} ${builtins.toString config.zerovpn.announceInterval} ${config.zerovpn.name} ${config.zerovpn.dnsPort}";
      };
    })
    (lib.mkIf config.zerovpn.server.enable {
      systemd.services.zerovpnServer = {
        ...
        script = "${pkgs.zerovpn}/bin/0vpn-root ${config.zerovpn.interface} /etc/zerovpn-key ${config.zerovpn.name} ${config.zerovpn.serverHost} ${builtins.toString config.zerovpn.endpointPort} ${builtins.toString config.zerovpn.announcePort} " + "\"" + (lib.concatStringsSep " " config.zerovpn.server.staticClients) + "\"" + ${config.zerovpn.dnsPort};
      };
    })
    ];
}
